...
## 26.02.14 최종 결론
"외부 상태(props)와 내부 상태(machine/reducer)를 어떻게 안전하게 동기화할 것인가?"라는 문제는 해결할 필요가 없다.
외부에서 제어될 수 있는 컴포넌트에서 닫힌 전이 규칙을 강제하는 도구(XState, 상태 머신 패턴의 reducer)를 사용하면 동기화 문제가 필연적으로 발생한다.
이 문제를 브릿지로 해결하려는 시도는 사실상 UI 전용 상태 머신 엔진을 만드는 것과 같다(Zag.js).
직접 만들 것이 아니라면, 훅 조합 + 순수 함수로 충분하며, 이쪽이 controlled 패턴과 자연스럽게 어울린다.
